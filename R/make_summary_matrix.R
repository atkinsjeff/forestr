...
make_summary_matrix <- function(df, m, method) {
  ...
  return(p)
  } else if(method == "MH"){  #declaring global varieties
    return_distance <- NULL
    ...
    # maximum value of VAI in the column
    d <- stats::setNames(stats::aggregate(lad ~ xbin, data = m, FUN = max, na.rm = FALSE, na.action = 'na.pass'), c("xbin", "max.lad"))

    # sum of VAI in column
    e <- stats::setNames(stats::aggregate(lad ~ xbin, data = m, FUN = sum, na.rm = FALSE, na.action = 'na.pass'), c("xbin", "sum.lad"))
    # standard deviation of VAI for column
    f <- stats::setNames(stats::aggregate(lad ~ xbin, data = m, FUN = stats::sd, na.rm = FALSE, na.action = 'na.pass'), c("xbin", "sd.lad"))

    # this is height at which max vai occurs
    g <- m$zbin[match(d$max.lad, m$lad)]
    g <- data.frame(g)
    colnames(g) <- c("max.lad.z")
    #print(g)

    #mean column leaf height that is the "heightBin" from Matlab code
    # first we make el
    #m$el <- (m$vai / m$sum.vai) * 100
    m$lad.z <- m$lad * (m$zbin + 0.5)
    h <- stats::setNames(stats::aggregate(lad.z ~ xbin, data = m, FUN = sum, na.rm = FALSE,  na.action = 'na.pass'), c("xbin", "lad.z.sum"))
    ...
    p$height.bin <- p$lad.z.sum / p$sum.lad
    p[is.na(p)] <- 0
